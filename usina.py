...
for video in grade_para_processar:
    ...
    periodo_dia = "nesta manhã" if horario == "06:00" else "neste meio-dia" if horario == "12:00" else "neste entardecer" if horario == "18:00" else "nesta noite"
    ...
    1. ESTRATÉGIA CENOURA/CHOCOLATE: O Título e a Thumb devem focar no "Chocolate" (o desejo/alívio imediato: dormir bem, afastar inimigo, paz no lar, milagre urgente). O roteiro entrega a oração profunda (Cenoura).
       -> REGRA DE TÍTULO: O título DEVE OBRIGATORIAMENTE começar com a dor/benefício. É ESTRITAMENTE PROIBIDO começar o título com a palavra "Oração". O período do dia ("- {titulo_sufixo}") entra APENAS no final.
    2. REGRA DOS 15 SEGUNDOS (HOOK 3A REFORMULADO): O início do roteiro DEVE ter 3 blocos rápidos:
       - Atenção (0-5s): Uma AFIRMAÇÃO EMPÁTICA sobre a dor do fiel. (PROIBIDO usar perguntas diretas. Ex: "Eu sei que o cansaço tem pesado nos seus ombros...").
       - Ambientação Sensorial (5-10s): Conecte a dor com o cenário de {periodo_dia} (Ex: "...mas sinta a brisa {periodo_dia} que nasce...").
       - Autoridade/Agenda (10-15s): Diga que {persona_prompt} tem uma palavra de libertação e peça para ficar até o final para sentir o alívio.
    3. CTA IMEDIATO: Peça naturalmente no início: "Se você crê, digite 'Amém, eu recebo' nos comentários agora mesmo".
    4. RESET DE ATENÇÃO (MEIO DO VÍDEO): Exatamente na metade do roteiro, insira uma frase falada para reconectar o espectador. (Ex: "Preste muita atenção agora... Não deixe que as distrações te afastem. Continue comigo, pois a bênção que clamaremos a seguir mudará o seu dia.").
    
    REGRAS GERAIS:
    5. PROIBIDO MENCIONAR HORÁRIOS EXATOS: Nunca diga "06:00" ou "18:00". Use apenas "{periodo_dia}".
    6. ARCO: Vulnerabilidade -> Súplica -> Entrega/Gratidão. Inclua um bloco pedindo pela saúde dos enfermos.
    7. PAUSAS: OBRIGATÓRIO usar abundantes pontos suspensivos (...) para forçar pausas na voz da IA.
    8. CENSURA: PROIBIDO descrições de violência física. PROIBIDO usar "Ah!", "Oh!".
    9. ANTI-JSON: Escreva em TEXTO PLANO. PROIBIDO JSON, chaves {{ }} ou asteriscos (*).
    {regra_persona}
    {regra_meditacao}
    
    FORMATO EXATO:
    TITULO: [Título magnético focado no benefício. FORMATO: "Frase sobre a dor/alívio - {titulo_sufixo}". SEM DATA. SEM ASTERISCOS]
